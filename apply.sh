...
## Wallpaper ---------------------------------
apply_wallpaper() {
    local wallpaper="$HOME/Pictures/wallpapers/mount.jpg"
 
    if [[ ! -f "$wallpaper" ]]; then
        echo "Error: El archivo de fondo '$wallpaper' no existe." >&2
        return 1
    fi
 
    local heads=$(xrandr --listmonitors | awk 'NR>1 {print $1}')
    for head in $heads; do
        nitrogen --head=$head --save --set-zoom-fill "$wallpaper" &>/dev/null || {
            echo "Error al aplicar el fondo en el monitor $head" >&2
        }
    done
}
